...
def extract(
    raw_headers: str, raw_alignment: str, raw_rows: str
) -> tuple[list[MarkdownType], list[str], list[list[str]]]:
    ...
    return rows, list(map(lambda column: loads(column)[0], columns)), list(map(match_alignment, alignment))
...
class Table(Markdown):
    ...
    def __init__(
        ...
        columns: t.Iterable[MarkdownType],
        alignment: t.Iterable[str] = (),
    ...
    def loads(  # type: ignore[override]  # pylint: disable=arguments-differ
        ...
        return cls(*extract(headers, alignment, rows), element_id=element_id)
